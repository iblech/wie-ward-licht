...
# -*- coding: utf-8 -*-
#
# Numerische Simulation von Mond- und Satellitenorbit mit dem
# Leapfrog-Integrator (anstatt dem expliziten Euler-Verfahren)
#
# * Mit "c" kann man die Kameraperspektive umschalten.
# * Mit einem Linksklick beschleunigt die Rakete (oranger Punkt) in die Richtung
#   des Klicks.
# * Mit der Oben- bzw. Untentaste verdoppelt bzw. halbiert man die
#   Simulationsgeschwindigkeit.
#
# Eine interessante Aufgabe besteht darin, die Rakete auf Rendezvous-Kurs mit
# der ISS (roter Punkt) zu bringen. Das naive Vorgehen, in die Richtung der ISS
# zu beschleunigen, scheitert nämlich. Was muss man stattdessen machen?
...
pygame.display.set_caption("Erdorbitexperimente")
...
# Einen halben Zeitschritt simulieren (das verlangt das Leapfrog-Verfahren)
def runFirstPhysicsStep(dt):
    for name in bodies:
        if name == "earth": continue
        bodies[name]["vel"] = bodies[name]["vel"] + dt * calcAcceleration(name) / 2

# Leapfrog-Integration. Das bedeutet: Zunächst die Positionen aktualisieren,
# dann die neuen Beschleunigungen berechnen und erst danach die
# Geschwindigkeiten aktualisieren.
#
# Was passiert, wenn man die Beschleunigen schon vorher berechnet?
# Was passiert, wenn man erst die Geschwindigkeiten und dann die Positionen
# aktualisiert?
def runPhysics(dt):
    for name in bodies:
        if name == "earth": continue
